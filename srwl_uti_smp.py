...
def srwl_opt_setup_smp_rnd_obj2d( #RAC06052020
        _thickness, _delta, _atten_len, _rx=0.02, _ry=0.02, _xc=0, _yc=0, _nx=1001, _ny=1001,
        _dens=1, _edge_frac = 0.02, _sim_step_size = 0.001,
        _obj_type = 1, _r_min_bw_obj = 0.002, 
    ...
    Parameters from user input
        ...
        :param _rx: range of the horizontal coordinate [m] for which the transmission is defined (0.02m = 20mm)
        :param _ry: range of the vertical coordinate [m] for which the transmission is defined (0.02m = 20mm)

    Parameters for "srwl_uti_smp_rand_obj2d.py"
        :param _dens = 1: approximate density [particles per mm^2]
        :param _particle_radius = 0.002: each particle radius [m] (0.0002m = 0.2 mm)
        :param _edge_fraction = 0.02: edge fraction; how close to the edge can an object be placed on the object plane.
        :param _simulation_step_size = 10: how large steps [m] that the simulation will take before trying to place a shape. (0.002m = 2mm) 
        :param _obj_type = 1: shape of objects. Choices: 1=rectangle, 2=ellipse, 3=polygon, 4=triangle, 5=random_shapes
        :param _r_min_bw_obj = 10: the minimum distance [m] between the centers of any two randomly placed shapes. (0.002m = 2mm) 
        ...
        :param _size_dist = 1: distribution of sizes. Choices are: 1=uniform, 2=normal(Gaussian), 3=Flory–Schulz
        :param _ang_min = 0: minimum rotation angle of shape [degrees].
        :param _ang_max = 0: maximum rotation angle of shape [degrees].
        :param _ang_dist = 1: distribution of rotation angle. Choices are: 1=uniform, 2=normal(Gaussian), 3=Flory–Schulz
        :param _rand_alg = 1: select randomization_algorithm.
            Random ~uniform seeding (_rand_alg = 1 #default): Algorithm will 
                ...
                2D random walk (_rand_alg = 2): Each object will be placed using
                a random 2D random walk.
        :param _obj_par1 = None: 
            rectangle: ratio of width to length (length is the longest dimension). 
                Leaving value as None will give 0.5 (1/2) as width to length 
                ratio unless _obj_par2 = True is selected to randomize the 
                width to length ratio.
            ellipse: ratio of minor to major semi-axes. 
                Leaving value as None will give 0.666~ (3/5) as minor to major 
            ...
            random shapes: Which shapes to randomly generate.
                Choices are [1='circle',2='rectangle',3='square',4='ellipse'
                ,5='triangle',6='polygon'].
    ...
    Parameters for "srwlib.SRWLOptT"
        :param _extTr = 0: (srwlib.SRWLOptT) transmission outside the grid/mesh is zero (0), or it is same as on boundary (1)
        :param _ne = 1: (srwlib.SRWLOptT) number of transmission data points vs photon energy
        :param _e_start = 0: (srwlib.SRWLOptT) initial value of photon energy
        :param _e_fin = 0: (srwlib.SRWLOptT) final value of photon energy

    Return object (image or transmission (SRWLOptT) type optical element)
        :return _ret='srw' return image (ong, jpg, or tiff) or transmission (SRWLOptT) type optical 
            element which simulates a random 2D disk.
    """
    ...
    #Load package matplotlib
        ...
        if(_ne > 1): # if number of transmission data points vs photon energy > 1
            if((isinstance(_delta, list) or isinstance(_delta, np.ndarray)) 
            and (isinstance(_atten_len, list) or isinstance(_atten_len, np.ndarray))):
        ...
        opT = srwlib.SRWLOptT(_nx=_nx, _ny=_ny, _rx=_rx, _ry=_ry, _arTr=arTr, _extTr=_extTr, _x=_xc, _y=_yc, _ne=_ne, _eStart=_e_start, _eFin=_e_fin)
        opT.input_parms = input_parms # add user input parameters to transmission optical element

        ### Return transmission (SRWLOptT) type optical element
        return opT